...
class Orders extends Model
{
    ...
    private function updateLine($order, $line, $taxes) {
        $ecritureComptable = [] ;
        ...
        if (isset($line->sublines) && count($line->sublines)) {
            foreach ($line->sublines as $subline) {
                $ecritureComptable = $this->fuisionTableTaxe($ecritureComptable, $this->updateLine($order, $subline, $taxes));
            }
        ...
        if ($order->global_discount > 0 && $line->id_parent == 0) {
            $ecritureComptable = $this->appliqueRemise($ecritureComptable, $order->global_discount);
        }
